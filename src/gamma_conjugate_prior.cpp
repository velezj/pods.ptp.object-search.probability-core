...
namespace probability_core {
  ...
  namespace testing_true {
		...
		const gamma_conjugate_prior_t& gcp )
    {
      ...
      mp_float a = gamma.shape;
      mp_float b = gamma.rate;
      mp_float p = gcp.p;
      mp_float q = gcp.q;
      mp_float r = gcp.r;
      mp_float s = gcp.s;
      
      mp_float p_pow = boost::multiprecision::pow( p, a - 1 );
      mp_float e = boost::multiprecision::exp( - b * q );
      mp_float g = boost::multiprecision::pow( boost::math::tgamma( a ), r );
      mp_float b_pow = boost::multiprecision::pow( b, - a * s );
      mp_float res = ( p_pow * e ) / ( g * b_pow );
      return res;
    }
